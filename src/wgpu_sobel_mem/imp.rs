...
use crate::glib;
use crate::gst_wgpu::{
    WgpuContext, WgpuMemory, WgpuMemoryAllocator, WgpuMemoryExt, WgpuMemoryRef,
...
use gst_video::prelude::VideoFilterExtManual;
...
impl BaseTransformImpl for WgpuSobelMem {
    ...
    fn transform(
        ...
        let mem = inbuf
            .iter_memories()
            .find_map(|x| x.downcast_memory_ref::<WgpuMemory>());
    ...
    fn propose_allocation(
        ...
        let allocator =
            WgpuMemoryAllocator::new(self.wgpu_context.lock().as_ref().cloned().unwrap());
